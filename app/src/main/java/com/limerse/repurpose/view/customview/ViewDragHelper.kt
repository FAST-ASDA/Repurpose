...
class ViewDragHelper private constructor(
    ...
    private fun saveLastMotion(ev: MotionEvent) {
        ...
        for (i in 0 until pointerCount) {
            ...
            if (mLastMotionX != null && mLastMotionY != null && mLastMotionX!!.size > pointerId && mLastMotionY!!.size > pointerId) {
                mLastMotionX!![pointerId] = x
                mLastMotionY!![pointerId] = y
            }
        }
            ...
            MotionEvent.ACTION_MOVE -> {
                ...
                while (i < pointerCount && mInitialMotionX != null && mInitialMotionY != null) {
                    ...
                    val toCapture = findTopChildUnder(
                        mInitialMotionX!![pointerId].toInt(), mInitialMotionY!![pointerId]
                            .toInt()
                    )
    ...
    private fun checkNewEdgeDrag(delta: Float, odelta: Float, pointerId: Int, edge: Int): Boolean {
        ...
        if (mInitialEdgesTouched[pointerId] and edge != edge || mTrackingEdges and edge == 0 || mEdgeDragsLocked[pointerId] and edge == edge || mEdgeDragsInProgress[pointerId] and edge == edge ||
